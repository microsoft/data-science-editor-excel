...
import { blocks, category, transforms, setCurrentWorkspace, currentWorkspace } from "./blocks";
...
(() => {
    ...
    Office.onReady(() => {
        loadSetting(SettingsKey.EditorSaveData)
            .then(setting => {
                loaded = true
                if (!setting) {
    ...
    async function onTableChanged(eventArgs: Excel.TableChangedEventArgs) {
        ...
        await Excel.run(async context => {
            ...
            if (currentDslId && eventArgs.worksheetId === sheet.id && table.name.charAt(0) !== "_") {
                post({
                    type: "dsl",
                    dslid: currentDslId,
                    action: "change"
