...
export async function isExistingTable(context: Excel.RequestContext, name: string, sheet: Excel.Worksheet): Promise<boolean> {
...
async function fillTableFromDataset(dataset, context: Excel.RequestContext, sheet: Excel.Worksheet) {
    ...
    for (const rowKey in dataset) {
        ...
        if (!columnNamesCreated) {
            let columnNames = Object.keys(dataset[rowKey]);
        ...
        resultValues.push([]);
        let columnValues = Object.values(dataset[rowKey]);
    ...
    let tableRange = sheet.getRangeByIndexes(0 /*startRow*/, 0 /*startCol*/, resultValues.length, resultValues[0].length);
    tableRange.load("values");
    await context.sync();
    tableRange.values = resultValues;
    await context.sync();

    let table = sheet.tables.add(tableRange, true /*hasHeaders*/);
    table.load("name");
    await context.sync();
