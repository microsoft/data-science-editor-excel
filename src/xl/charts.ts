...
const clearAllSeriesFromChart = async(context: Excel.RequestContext, chart: Excel.Chart) => {
...
const getRangeForSelectedColumn = async (context: Excel.RequestContext, sheet: Excel.Worksheet, selectedTable: string, columnName: string) => {
  ...
  for (let i = 0; i < tableRange.columnCount; i++) {
    ...
    if (currentCol.values[0][0] == columnName) {
      return sheet.getRangeByIndexes(rowIndex + 1, columnIndex, tableRange.rowCount - 1, 1);
    }
  }
...
export const insertBarChart = async (selectedTable: string, selectedIndex: string, selectedValue: string) => {
  Excel.run(async (context) => {
    if (selectedTable === undefined || selectedIndex === undefined || selectedValue === undefined) {
      // Not enough information, so let's bail
      return;
    }

    let exportSheet = context.workbook.worksheets.getItemOrNullObject(DATA_SCIENCE_EXPORT_SHEET);
    await context.sync();
    ...
    let seriesRange = await getRangeForSelectedColumn(context, exportSheet, selectedTable, selectedValue);
    let categoryNamesRange = await getRangeForSelectedColumn(context, exportSheet, selectedTable, selectedIndex);
